...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['city'])) {$text = $_POST['city'];}
    if (isset($_POST['text'])) {$text = $_POST['text'];}
    if (isset($_POST['zbroya'])) {$zbroya = $_POST['zbroya'];}
    if (isset($_POST['ato'])) {$ato = $_POST['ato'];}
    if (isset($_POST['formData'])) {$formData = $_POST['formData'];}

    $to = "galkinpr@gmail.com"; /*Укажите адрес, на который должно приходить письмо*/
    ...
    $subject = "$formData";
    $message = "$formData<br> <b>Ім'я:</b> $name <br><b>Місто:</b> $city <br><b>Повідомлення:</b> $text <br><b>Телефон:</b> $phone  ";
